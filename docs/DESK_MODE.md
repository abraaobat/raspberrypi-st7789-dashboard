...
A instalação v0.5.0 foi confirmada no Raspberry em 17/09/2026, com auditoria automática aprovada. Faça um ciclo de 1 minuto no painel, confirme a contagem e o estado final no TFT, pause/retome e confira Relógio com os botões. Esses testes visuais não foram registrados como homologados. A biblioteca de modelos v0.6.0 é uma atualização separada e não muda o comportamento do timer; veja [CUSTOM_TEMPLATES.md](CUSTOM_TEMPLATES.md).

...
- SSD1306/ILI9341 have optional physical-driver implementations since v0.8.0, but are experimental, not physically validated. Profile simulation does not change the reference ST7789 hardware. Never present software tests as hardware certification.
...
- Keep the v0.3.0 physical-validation badge separate from newer software additions. The active v0.6.0 installation and health checks on the Raspberry Pi were confirmed on 17/09/2026, not the visual validation of newer pages or real connectors. v0.8.0 adds MQTT short subscriptions/text/JSON, private credentials, safe profile simulation and optional experimental drivers; v0.7.0 Docker remains optional. MQTT prefers retained values, is not continuous event delivery and cannot infer measurement age. Fake-service tests and isolated Mosquitto interoperability do not establish validation of the maintainer's broker, devices, Docker, Pi-hole/Home Assistant or TFT. Manual tests are deferred to the final checklist. Do not install broker/Docker or grant privileges automatically. Preserve all photos and the confirmed Pix destination.
